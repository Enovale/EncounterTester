        ...
        static EncounterHelper()
        {
            ...
            foreach (var chapterData in StaticDataManager.Instance.partList.list)
            {
                ...
                foreach (var subchapterData in chapterData.subchapters)
                {
                    ...
                    foreach (var stageNodeInfo in subchapterData.StageNodeList)
                    {
                        ...
                        {
                            var dungeonData = StaticDataManager.Instance.DungeonMapList.GetDungeonData(stageNodeInfo.StageId);
                            if (dungeonData == null)
                                continue;
                            
                            var dungeonName = TextDataManager.Instance.StageNodeText.GetData(stageNodeInfo.NodeId).GetTitle();
                                ...
                                foreach (var sector in floor.sectors)
                                {
                                    for (var i = 0; i < sector.Nodes.Count; i++)
                                    {
                                        ...
                                        dungeonEncounterList.Add(new()
                                        {
                                            ...
                                            ProgressType = stage!.StageType == STAGE_BATTLE_TYPE.Abnormality
                                                ? STAGE_PROGRESS_TYPE.STORY_AB_BATTLE
                                                : STAGE_PROGRESS_TYPE.STORY_BATTLE
                                        });
                                    }
                                }
                            }
                        ...
                        else
                        {
                            ...
                            encounterData.Add(new()
                            {
                                ...
                                StageData = stage,
                                StageType = STAGE_TYPE.NORMAL_BATTLE,
                                ProgressType = stage.StageType == STAGE_BATTLE_TYPE.Abnormality ? STAGE_PROGRESS_TYPE.STORY_AB_BATTLE : STAGE_PROGRESS_TYPE.STORY_BATTLE
                            });
                        }
                    }
                }
            ...
            for (var i = 0; i < expList.Count; i++)
            {
                ...
                expEncounters.Add(new()
                {
                    ...
                    ProgressType = STAGE_PROGRESS_TYPE.EXP_DUNGEON
                });
            }
            ...
            foreach (var threadDungeonData in threadList)
            {
                ...
                foreach (var threadStage in threadDungeonData.SelectStage)
                {
                    threadEncounters.Add(new()
                    {
                        ...
                        ProgressType = STAGE_PROGRESS_TYPE.THREAD_DUNGEON
                    });
                }
            }
            ...
            foreach (var railwayDungeonData in railwayList)
            {
                ...
                foreach (var dungeonSector in railwayDungeonData.Sector)
                {
                    data.Item2.Add(new()
                    {
                        Name = TextDataManager.Instance.RailwayDungeonStationName.GetData(railwayDungeonData.ID).GetStationName(dungeonSector.NodeId),
                        StageData = StaticDataManager.Instance.GetDungeonStage(dungeonSector.StageId, default, DUNGEON_TYPES.RAILWAY_DUNGEON),
                        StageType = STAGE_TYPE.RAILWAY_DUNGEON,
                        ProgressType = dungeonSector.IsAbBattleIcon ? STAGE_PROGRESS_TYPE.RAILWAY_DUNGEON_AB_BATTLE : STAGE_PROGRESS_TYPE.RAILWAY_DUNGEON_BATTLE
                    });
                }
